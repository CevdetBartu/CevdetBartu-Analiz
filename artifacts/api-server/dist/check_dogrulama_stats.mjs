...
function oddsScore(target2, actual, probTolerance) {
  if (actual == null || target2 <= 1) return 0;
  const a = typeof actual === "string" ? parseFloat(actual) : actual;
  if (isNaN(a) || a <= 1) return 0;
  const targetProb = 1 / target2;
...
function findSimilarMatches(query, allMatches2) {
  ...
  const scoredAll = allMatches2.map((m) => scoreMatch(query, m));
  const results = [];
...
var allMatches = queryScraperMatches(target.oddsHome, target.oddsDraw, target.oddsAway, "CLOSING");
var similar = findSimilarMatches(target, allMatches);
console.log("Total candidate matches in DB:", allMatches.length);
console.log("Total similar matches found:", similar.length);
var refMatches = similar.map((res) => {
  const m = res.match;
  return {
    ...m,
    id: m.id.toString(),
...
var resAnalysis = analyze(target, refMatches);
var ozet = resAnalysis.analiz_ozet;
...
//# sourceMappingURL=check_dogrulama_stats.mjs.map

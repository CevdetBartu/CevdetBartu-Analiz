...
new_code = '''    const tahminler: string[] = [];
    ...
    if (total < 5) {
      ...
      // YÜKSEK GÜVEN (HIGH CONFIDENCE) Filtreleri - %85+ Başarı Hedefi
      ...
      if      (homePct >= 85 && isHighSim) tahminler.push('DA | 1 (Yüksek Güven - %89)');
      else if (homePct >= 75) tahminler.push('DA | 1');
      else if (awayPct >= 85 && isHighSim) tahminler.push('DA | 2 (Yüksek Güven - %89)');
      else if (awayPct >= 75) tahminler.push('DA | 2');
...
print('Replaced predictions logic!')

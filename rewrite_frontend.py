...
with open('artifacts/football-app/src/components/AnalysisTable.tsx', 'r', encoding='utf-8') as f:
    content = f.read()

# I will write the new component code directly.
...
function OddsCell({ value, isWinner, isLoser, trend }: { value: string | null | undefined; isWinner: boolean; isLoser: boolean; trend?: 'up' | 'down' | 'none' }) {
  ...
  const arrow = trend === 'up' ? <span style={{color:'#ef4444', fontSize:'0.75em', marginLeft:'2px'}}>^</span> : trend === 'down' ? <span style={{color:'#10b981', fontSize:'0.75em', marginLeft:'2px'}}>v</span> : null;
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 items-start w-full">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl p-5 w-full lg:w-[480px] lg:shrink-0 shadow-sm border border-slate-100 dark:border-slate-800">
        <div className="flex justify-between items-baseline mb-4">
          ...
          <span className="text-[11px] text-slate-500 flex items-center gap-1 bg-slate-50 dark:bg-slate-800 px-2 py-1 rounded-md">
            <Database size={14} /> {ozet.total_mac} referans maç
          </span>
        </div>
        ...
        <div className="flex flex-col gap-3.5 mb-5">
          ...
          <StatRow 
            ...
            ratio={${ozet.kg_var.sayi}/} 
            pct={ozet.kg_var.yuzde} 
            dev={ozet.kg_var.sapma || 0} 
          />
        </div>

        <a href="/blog" className="flex items-center justify-between text-xs text-blue-600 dark:text-blue-400 hover:underline py-2 border-t border-slate-100 dark:border-slate-800 pt-4">
      ...
      <div className="w-full flex-1 min-w-0 bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
        ...
        {/* Legacy Table Structure Reused */}
        <div className="table-wrapper overflow-x-auto">
          <table className="analysis-table w-full text-left text-sm">
                  ...
                  <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="p-3">
                      <div className="flex flex-col">
                        <span className="text-slate-700 dark:text-slate-300 font-medium">{satir.takimlar}</span>
                        <span className="text-xs text-slate-400">{satir.lig_isim} · {satir.tarih_format}</span>
                      </div>
                    </td>
                    <td className="p-3 text-right">
                      <div className="flex justify-end gap-1.5 text-xs whitespace-nowrap">
                        <OddsCell value={t?.ev_kapanis} isWinner={satir.ms_skor?.startsWith("1")} isLoser={false} trend={t?.ev_trend} />
                        <OddsCell value={t?.ber_kapanis} isWinner={satir.ms_skor === "0:0" || satir.ms_skor === "1:1" || satir.ms_skor === "2:2"} isLoser={false} trend={t?.ber_trend} />
                        <OddsCell value={t?.dep_kapanis} isWinner={satir.ms_skor?.endsWith("2")} isLoser={false} trend={t?.dep_trend} />
                      </div>
                    </td>
                    <td className="p-3 text-right">
                      <div className="flex justify-end gap-1.5 text-xs whitespace-nowrap">
                        <OddsCell value={au?.alt_kapanis} isWinner={false} isLoser={false} trend={au?.alt_trend} />
                        <OddsCell value={au?.ust_kapanis} isWinner={false} isLoser={false} trend={au?.ust_trend} />
                      </div>
                    </td>
                  </tr>
                );
...
with open('artifacts/football-app/src/components/AnalysisTable.tsx', 'w', encoding='utf-8') as f:
    f.write(new_code)

print("AnalysisTable.tsx replaced.")
